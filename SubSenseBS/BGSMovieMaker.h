...
public:
		...
		int saveImg = 0, int type = 0, int fps = 25)
	{
		...
		cv::Size bsize(size.width * 2, size.height);
		writer.open(std::string(outFileName), CV_FOURCC('X', 'V', 'I', 'D'), fps, bsize);
		...
		for (int i = from; i <= to; i++)
		{
			...
			for (int i = 0; i<img.rows; i++)
			{
				for (int j = 0; j<img.cols; j++)
				{
					...
					idx_uchar_rgb = (i*bimg.cols + j*2) * 3;
					uchar* imgPtr2 = bimg.data + idx_uchar_rgb;
					if (type == 0)
					{
						if (*mPtr1 != 0xff)
						{
			...
			char text[200];
			sprintf(text, "%s", method1);
			cv::putText(bimg, text, cv::Point(10, 20), CV_FONT_ITALIC, 1, CV_RGB(255, 215, 0));
			sprintf(text, "%s", method2);
			cv::putText(bimg, text, cv::Point(img.cols + 10, 20), CV_FONT_ITALIC, 1, CV_RGB(255, 215, 0));
			writer << bimg;
