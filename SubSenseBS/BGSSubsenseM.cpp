...
void BGSSubsenseM::LocalSearch(const cv::Mat& img, const int x, const int y,  int& wx,  int& wy,  int s)
{
	//search around (wx,wy),find a pixel best match the bg model
	int matchedMax = 0;
	double distMin = 1e10;
	int optX(wx),optY(wy);
	...
	for(int m=-s; m<=s; m++)
	{
		for(int n=-s; n<=s; n++)
		{
			int rx = m+wx;
			int ry = n+wy;
			if (rx < 2 || ry < 2 || rx+2 >= m_oImgSize.width-1 || ry+2 >= m_oImgSize.height)
				continue;
	...
	/*	if (optX != wx || wy != optY)
	std::cout<<"optx "<<optX<<" wx "<<wx<<" opty "<<optY<<" wy "<<wy<<std::endl;*/
	wx = optX;
	wy = optY;
...
{
	WarpBasedOperator(_image,_fgmask,learningRateOverride);
	return;
	//EstimateHomos();
...
failedcheck1ch:
	...
	{ 
		...
		for(size_t k=0; k<m_nKeyPoints; ++k) {
			...
			{					
				...
				size_t anCurrIntraLBSPThresholds[3]; 
				for(size_t c=0; c<3; ++c) {
					const uchar nCurrBGInitColor = oInputImg.data[oidx_uchar_rgb+c];
					m_oLastColorFrame.data[oidx_uchar_rgb+c] = nCurrBGInitColor;
					anCurrIntraLBSPThresholds[c] = m_anLBSPThreshold_8bitLUT[nCurrBGInitColor];
					LBSP::computeSingleRGBDescriptor(oInputImg,nCurrBGInitColor,x,y,c,m_anLBSPThreshold_8bitLUT[nCurrBGInitColor],((ushort*)(w_oLastDescFrame.data+oidx_ushrt_rgb))[c]);
	...
	cv::swap(m_dx,m_preDx);
	cv::swap(m_dy,m_preDy);
	refreshModel(outMask,0.1);
}
