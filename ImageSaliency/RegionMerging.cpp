...
void HandleOcculusion(const cv::Mat& img, SuperpixelComputer& computer, const char* outPath, std::vector<int>& newLabels, std::vector<RegionSalInfo>& regInfos, std::vector<SPRegion>& regions, int * segment, bool debug = false)
{
	...
	int minSId, minBId,minRegInfoId;
	float threshold(0.4);
	int idx(0);
	...
	for (size_t i = 0; i < regInfos.size() - 1; i++)
	{
		...
		for (size_t j = i + 1; j < regInfos.size(); j++)
		{
				...
				isNeighbor(regions, id, nid))
			{
				...
				float wDist = wc*colorDist + wa*ad2cDis;
				if (wDist < minDist)
				{
					minDist = wDist;
					minSId = id;
	...
	while (regSize > 2 && minDist < threshold)
	{
		...
		if (debug)
		{
			...
			pair.push_back(make_uint2(minSId, minBId));
			GetRegionMap(img.cols, img.rows, &computer, newLabels, regions, pair, rmask);
			sprintf(name, "%s%dHOccMerge0_%d_%d_Region_%2d_%d.jpg", outPutPath, idx, minSId, minBId, (int)(minDist * 100), regSize);

			cv::imwrite(name, rmask);
		}

		MergeRegions(minSId, minBId, newLabels, spPoses, regions);
		...
		regInfos.erase(minRegInfoId + regInfos.begin());
		//RegDists.clear();
		minDist = 255;
		for (size_t i = 0; i < regInfos.size() - 1; i++)
		{
			...
			for (size_t j = i + 1; j < regInfos.size(); j++)
			{
					...
					(regions[id].edgeSpNum > 1 && regions[nid].edgeSpNum > 1))
				{
					...
					//float ad2cI = sqrt(sqr(regions[id].ad2c.x) + sqr(regions[id].ad2c.y));
					//float  = abs(ad2cI - ad2cJ);
					//rd.edgeness = ad2cDis;
					//RegDists.push_baad2cJ = sqrt(sqr(regions[nid].ad2c.x) + sqr(regions[nid].ad2c.y));
					//float ad2cDisck(rd);
					float cDist = RegionColorDist(regions[id], regions[nid]);
					float ad2cI = sqrt(sqr(regions[id].ad2c.x) + sqr(regions[id].ad2c.y));
					float ad2cJ = sqrt(sqr(regions[nid].ad2c.x) + sqr(regions[nid].ad2c.y));
					float ad2cDis = abs(ad2cI - ad2cJ);
					float wDist = wc*cDist + wa*ad2cDis;
					if (wDist < minDist)
					{
						minDist = wDist;
						minSId = id;
...
float SLICRegionBoxLocalContrast(SuperpixelComputer& computer, std::vector<int>& nLabels, std::vector<SPRegion>& regions, int rid, HISTOGRAMS& colorHist, float theta)
{
	int spSize(0);
	int * labels(NULL);
	SLICClusterCenter* centers(NULL);
...
#pragma omp parallel for	
	...
	//����ͼ���б߿�����,��ʱ�߿��ռ����ȫ��border��ɱ�����borderratioΪ0
	//���߿�ϲ�������������ھ�
	/*if (regInfos.size() ==3)
	{
...
void SLICSaliencyGuidedRegionGrowing(const char* workingPath, const char* imgFolder, const char* outputPath, const char* imgName, const cv::Mat& img, const cv::Mat& edgeMap, SuperpixelComputer& computer, cv::Mat& salMap, int regThreshold, bool debug)
{
	char outPath[300];
	char fileName[300];
	...
	while (1)
	{
		...
		if (RegionSaliency(img.cols, img.rows, colorHist, outPath, &computer, newLabels, regions, regInfos, debug))
		{
			...
			for (size_t i = 0; i < regInfos.size() - 1; i++)
			{
				...
				pixels += regions[id].pixels;				
				box = MergeBox(box, regions[id].Bbox);
			}
