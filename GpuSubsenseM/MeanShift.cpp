...
int MeanShift(const cv::Mat& imgMat, int **labels,const int spatial_radius, const double color_radius)
{
	DECLARE_TIMING(timer);
	START_TIMING(timer);
	IplImage imgD = imgMat;
	IplImage* img = &imgD;
...
int MeanShift(const cv::Mat& imgMat, int *&labels,const int spatial_radius, const double color_radius)
{
	...
	for(int i=0;i<img->height;i++) 
		...
		float *mode = new float[img->height*img->width*3];
		{
			...
			for(int i=0;i<img->height;i++) 
				for(int j=0;j<img->width;j++)
					labels[i*img->width+j] = -1;
			for(int i=0;i<img->height;i++) 
				for(int j=0;j<img->width;j++)
					if(labels[i*img->width+j]<0)
					{
						labels[i*img->width+j] = ++label;
						float L = (float)((uchar *)(result->imageData + i*img->widthStep))[j*result->nChannels + 0],
							U = (float)((uchar *)(result->imageData + i*img->widthStep))[j*result->nChannels + 1],
							V = (float)((uchar *)(result->imageData + i*img->widthStep))[j*result->nChannels + 2];
						mode[label*3+0] = L*100/255;
						...
						while(!neighStack.empty())
						{
							...
							{
								int i2 = p.x+dxdy[k][0], j2 = p.y+dxdy[k][1];
								if(i2>=0 && j2>=0 && i2<img->height && j2<img->width && labels[i2*img->width+j2]<0 && color_distance(result, i,j,i2,j2)<color_radius2)
								{
									labels[i2*img->width+j2] = label;
									neighStack.push(cvPoint(i2,j2));
									modePointCounts[label]++;
									L = (float)((uchar *)(result->imageData + i2*img->widthStep))[j2*result->nChannels + 0];
									U = (float)((uchar *)(result->imageData + i2*img->widthStep))[j2*result->nChannels + 1];
		...
		for(int counter = 0, deltaRegionCount = 1; counter<5 && deltaRegionCount>0; counter++)
		{
			...
			for(int i=0;i<img->height;i++) 
				for(int j=0;j<img->width;j++)
				{
					if(i>0 && labels[i*img->width+j]!=labels[(i-1)*img->width+j])
					{
						// Get 2 free node
						raNode1			= freeRAList;
						raNode2			= freeRAList->next;
						...
						freeRAList		= freeRAList->next->next;
						// connect the two region
						raNode1->label	= labels[i*img->width+j];
						raNode2->label	= labels[(i-1)*img->width+j];
						if(raList[labels[i*img->width+j]].Insert(raNode2))	//already exists!
							freeRAList = oldRAFreeList;
						else
							raList[labels[(i-1)*img->width+j]].Insert(raNode1);
					}
					if(j>0 && labels[i*img->width+j]!=labels[i*img->width+j-1])
					{
						// Get 2 free node
						raNode1			= freeRAList;
						raNode2			= freeRAList->next;
						...
						// connect the two region
						raNode1->label	= labels[i*img->width+j];
						raNode2->label	= labels[i*img->width+j-1];
						if(raList[labels[i*img->width+j]].Insert(raNode2))
							freeRAList = oldRAFreeList;
						else
							raList[labels[i*img->width+j-1]].Insert(raNode1);
					}
				}
				...
				for(int i = 0; i < img->height; i++)
					for(int j = 0; j < img->width; j++)
						labels[i*img->width+j]	= label_buffer[raList[labels[i*img->width+j]].label];
...
int MeanShift(const IplImage* img, int **labels,const int spatial_radius,  const double color_radius)
{
	DECLARE_TIMING(timer);
	START_TIMING(timer);
